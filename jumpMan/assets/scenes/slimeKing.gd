...
func forwardJump():
	print("forward jump")
	$soundAngery.play()
	$Sightline.set_collision_mask_value(0, false)
	...
	$Sightline.set_collision_mask_value(0, true)
...
func _on_crown_body_entered(body):
	...
	if crownHealth >= 1 and crownIsAlive:
		...
		$crown.set_collision_mask_value(5, false)
		crownIsAlive = false
		crownDied()
...
func _on_AI_trigger_body_entered(body):
	...
	$actionTimer.start(tickCycle)
	$"AI trigger".set_collision_mask_value(0, false)
	print('start')
